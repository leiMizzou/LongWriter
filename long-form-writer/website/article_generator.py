...
def get_user_input() -> Tuple[str, int, str, str, Optional[str], Optional[str]]:
    """
    获取用户输入：标题、长度、体裁、语言、可选上下文和可选模板
    
    Returns:
        Tuple[str, int, str, str, Optional[str], Optional[str]]: 标题、长度、体裁、语言、上下文和模板文件
    """
    ...
    parser.add_argument("--template", type=str, default=None, help="可选大纲模板JSON文件路径，用于自定义文章结构")
    args = parser.parse_args()
    return args.title, args.length, args.genre, args.language, args.context, args.template

def load_outline_template(template_path: str, target_length: int) -> List[Dict[str, Any]]:
    """
    从JSON文件加载大纲模板
    ...
    Returns:
        List[Dict[str, Any]]: 加载的大纲数据
        
    Raises:
        ValueError: 如果模板格式不正确或无法读取
    """
    try:
        ...
        # 验证大纲结构
        validate_outline(outline, target_length)
        ...
        logging.info(f"成功从 {template_path} 加载大纲模板")
        return outline
    except json.JSONDecodeError as e:
        logging.error(f"模板JSON解析失败: {e}")
        raise ValueError(f"无法解析模板文件 {template_path}: {e}")
    except FileNotFoundError:
        logging.error(f"找不到模板文件: {template_path}")
        raise ValueError(f"找不到模板文件: {template_path}")
    except Exception as e:
        logging.error(f"加载模板时发生错误: {e}")
        raise ValueError(f"加载模板错误: {e}")
...
def main():
    ...
    try:
        ...
        title, length, genre, language, context, template_path = get_user_input()
        log_file = setup_logging(title)
        logging.info(f"开始为 {title} 生成文章")
        logging.info(f"日志文件创建于: {log_file}")
        
        # 根据是否提供模板决定使用模板还是生成大纲
        if template_path:
            try:
                logging.info(f"使用模板文件: {template_path}")
                outline = load_outline_template(template_path, length)
            except ValueError as e:
